...
in {
  ...
  */
  ...
  generatePage = utils.documentedFunction {
    ...
    examples = [
      (utils.mkExample {
        literalCode = ''
          styxlib.generation.generatePage {
        ...
        '';
        code = styxlib.generation.generatePage {
          layout = template: "<html><body>${template}</body></html>";
          template = page: ''
            <h1>Styx example page</h1>
            ${page.content}
  ...
  */
  ...
  mkSite = utils.documentedFunction {
    ...
    arguments = {
      ...
      genPageFn = {
        ...
        type = "Page -> String";
        default = l.literalExpression "styxlib.generation.generatePage";
      };
      pagePathFn = {
        description = "Function to generate a page from a page attribute set.";
        type = "Page -> String";
        default = l.literalExpression "page: page.path";
      };
    };
    ...
    examples = [
      (utils.mkExample {
        literalCode = ''
          styxlib.generation.mkSite { pageList = [ pages.index ]; }
        '';
      })
    ];
    ...
    function = {
      ...
      preGen ? "",
      postGen ? "",
      genPageFn ? styxlib.generation.generatePage,
      pagePathFn ? (page: page.path),
    }: let
      env = {
        meta = {platforms = l.platforms.all;} // meta;
    ...
    in
      nixpkgs.runCommand name env ''
        ...
        run_subs () {
          ...
          set: let
            key = l.head (l.attrNames set);
            value = l.head (l.attrValues set);
          in ''
            substituteInPlace subs \
              --subst-var-by "${key}" "${toString value}"
          ''
        ) (utils.setToList substitutions)}
        }
  ...
  */
  ...
  pagesToList = utils.documentedFunction {
    ...
    examples = [
      ...
      (utils.mkExample {
        literalCode = ''
          styxlib.generation.pagesToList {
            pages = {
              foo = { path = "/foo.html"; };
        ...
        '';
        code = styxlib.generation.pagesToList {
          pages = {
            foo = {path = "/foo.html";};
            bar = [{path = "/bar-1.html";} {path = "/bar-2.html";}];
          };
    ...
    function = {
      ...
      pages' = l.attrValues pages;
    in
      l.fold (
        p: acc:
          if l.isList p
          then acc ++ (map (x: l.recursiveUpdate default x) p)
          else if utils.is "pages" p
          then acc ++ (map (x: l.recursiveUpdate default x) p.pages)
          else acc ++ [(l.recursiveUpdate default p)]
      ) []
      pages';
  };
  ...
  */
  ...
  localesToPageList = utils.documentedFunction {
    ...
    arguments = {
      ...
      default = {
        ...
        type = "Locale -> Attrs";
        default = l.literalExpression "locale: {}";
      };
    };
    ...
    examples = [
      (utils.mkExample {
        literalCode = ''
          pagelist = styxlib.generation.localesToPageList {
            inherit locales;
            default = locale: {
      ...
      (utils.mkExample {
        literalCode = ''
          styxlib.generation.localesToPageList {
            locales = {
              eng = rec {
                code   = "eng";
                prefix = "/''${code}";
        ...
        '';
        code = styxlib.generation.localesToPageList {
          locales = {
            eng = rec {
              code = "eng";
              prefix = "/${code}";
