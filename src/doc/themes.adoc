...
`conf.nix` should be an attribute set defining theme options with a default value.
...
- `conf.themes.id`: `id` is the theme id, this holds the theme default configuration as it is declared in `theme.nix`.
====

[source, nix]
.Usage of a theme configuration interface in a **site** conf.nix
----
{
