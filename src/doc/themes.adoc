...
Styx themes that are part od the `styx-themes` package set are documented in the link:./appendix.html[appendix].
...
WARNING: `types.submodule` type checking is not supported.
...
`meta` attributes commonly used for the documentation are:
