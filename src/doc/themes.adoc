...
Theme meta information should be declared in the `meta.nix` file as an attribute set. Themes *must* define a `meta.nix` with a `name` attribute so the theme configuration is correctly processed. +
...
IMPORTANT: Themes *must* define a `meta.nix` with a `name` attribute.
