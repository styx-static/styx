...
Themes can be downloaded in the `themes` directory with git.

[source, shell]
.Downloading the showcase theme via git
----
...
$ cp $(nix-build -A styx-themes.showcase '<nixpkgs>')/templates/layout.nix themes/templates/layout.nix # <2>
----
