...
NOTE: In case `meta.name` and `meta.version` are not present, the site derivation will use a generic `styx-site` name.

WARNING: `meta.name` and `meta.version` must not use spaces.
