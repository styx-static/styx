...
- that evaluate each page set by evaluating `page.layout (page.template page)` and output the result in `page.href`.
...
rec {
  ...
  /* Library loading
  */
  lib = import styx.lib args; # <1>

  /* Configuration loading
  */
  conf = lib.utils.mergeConfs [ # <2>
  ...
  */
  templates = lib.themes.loadTemplates { # <3>
    inherit themes;
  ...
  */
  files = lib.themes.loadFiles { inherit themes; }; # <4>
----

<1> Load the styx library, library functions are documented in <<Library>>.
<2> Generate the configuration by merging the themes configuration, `conf.nix` and the `extraConf`, for details see <<Configuration>>.
<3> Loads the templates provided by the themes.
<4> Loads the static files provided by the themes.
