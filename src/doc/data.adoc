...
`env` parameter used in `loadFile` or `loadDir` function is insered in the scope, so `lib` functions or templates can be called. +
...
NOTE: `{{` and `}}` can be escaped by prepending a `\`, `\{{` and `\}}`, to prevent nix evaluation.
...
It is possible to split markup file in multiple pages by using the `<<<` separator.
