...
Data gets included in the default template environment and can also be used in the pages section of `site.nix` to attach data to page attribute sets.
...
- `loadDir`: to load all the files in a directory and return them as a list of attributes set. (non-recursive)
...
NOTE: The data section is only responsible for generating the data attribute set. Transforming the data attribute sets in a page attribute set is done in the pages section. +
...
NOTE: This example use `pages` and not `data` attribute sets because data attribute sets do not have a `href` field making it impossible to generate links to them. +
