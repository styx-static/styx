...
[[v0.7.0]]
=== Styx 0.7.0 - 2017/XX/XX

==== Highlights:

* Styx use its own version without relying on global `<nixpkgs>`, this enforce that `site,nix` format, used styx-themes and styx versions are matching.
...
* Themes matching are tied to the styx version avoiding versions mismatchs.
